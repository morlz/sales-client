...
<div class="tableWrapper">
	<div class="tableCoreWrapper">
		<table>
			...
			<virtual-scroller :items="sortedRows" item-height="50" content-tag="tbody">
				<template slot-scope="row">
					<tr class="hoverShow" v-if="row.item">
						<td v-if="lineNumbers && !minify" class="tableIndex" @click="clickHandler($event, row.item, 0)">{{ index + 1 }}</td>
						<td
							v-for="column, columnIndex in columns"
							:class="column.type"
							@click="clickHandler($event, row.item, columnIndex)"
							:style="{
						...
						<td class="buttons" v-if="local_buttonsCondition(row.item)">
